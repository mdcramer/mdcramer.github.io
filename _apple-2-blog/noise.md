...
Taking of break from exploring k-means on my <span class="no-break">Apple ]\[+</span>, I decided to takle of problem of significantly greater import: the speaker is really loud. I don't recall this ever bothering my in high school, but it's pretty loud and there is no way to non-programatically control the volume.

## It goes to 11

There's not a lot of how-to retro hardware upgrade videos on the internet but I found this and and it did the trick.

{% include video id="rO5eXxVxK_4" provider="youtube" %}

While it's a little difficult to follow (he's offers a schmetic but it's hard to tell which wires are going where), he links to the necessary parts. In addition to a 1kΩ audio log potentiometer, I got a knob that [goes to 11](https://youtu.be/uMSV4OteqBE?si=e2hANo3D6ktuzAxs&t=78){:target="_blank"} and files to 3D print a little part mount the knob on the side of the machine by sliding it into one of the air fonts.
...
Attempting to follow the video, my first instinct was the "parallel shunt" method, which has the potentiometer run side-by-side with the internal 8Ω speaker. I got a little confused by what connects to where, so I took a look at the speaker jack on the motherboard and, unsing a multimeter, tried to find ground. One of the pins registered 5.8Ω and the other 0, but ground was on the opposite side than I expected. So, to get a sense of how the neighboring capacitor is wired, I decided to consult the motherboard's schematic which of course, is included as a foldout at the back of the Apple ]\[ Reference manual.

<div class="image-grid" markdown="1">
  [![Speaker jack and neighbording capacitor on the motherboard](/assets/images/apple2/motherboard-speaker-jack.jpg)](/assets/images/apple2/motherboard-speaker-jack.jpg "Speaker jack and neighbording capacitor on the motherboard")
...
Since the inline series method only requires two terminals, Terminal 1 (the far-left pin when facing the shaft) was left empty. With the help from a lovely assitant (who, frankly, I could have used when connecting the header), I soldered jumper wires to the potentiometer. (I orignially added one to each terminal for the parallel shunt but later removed the one from terminal 1.)
...
From there it was easy. I chose the colors of the jumper cables to line up with the 4-pin jumper wire, but even if I got it backwards the volumn knob would just work in the opposite direction. (I didn't get it backwards.) Sliding the 3D printed mount into the case was easy although I had to bend the terminals pretty severely in order to get them around the floppy drive. And... guess what?
...
As anticipated, the dail does not have a full sweep of volume control. At 11 the sound is as lound as it's always been but somewhere around 7 it's pretty much at the lowest level of volume, which is pretty quiet, but it doesn't go completely silent. All of that is fine. It's operating as intended and doing the job I want it to do. The turning feels solid and for almost half the knob the audio softens nicely. It's also easy to remove and a simple jumper cable connecting a couple plugs on the 4-pin jumper wire will get it back to original operation. Most importanly, there is virtually no risk to the motherboard.

Not this there's a lot of chance of this happening but now at some point it'd be nice to find time to finish [Ultima 1](https://mortalwayfare.com/in-the-beginning-there-was-basic/){:target="_blank"}!